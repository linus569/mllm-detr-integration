...
def method1_sliding_window(processor, input_ids_np: np.ndarray) -> np.ndarray:
    ...
    for i in range(len(input_ids_np)):
        ...
        if len(seq) >= tokenized_prompt_len:
            windows = np.lib.stride_tricks.sliding_window_view(seq, tokenized_prompt_len)
            match_idx = np.where((windows == processor.tokenized_start_prompt).all(axis=1))[0]
            if match_idx.size > 0:
...
def test_performance_comparison(batch_size: int, seq_length: int):
    ...
    print(f"Speed difference: Method 2 is {time1/time2:.2f}x {'faster' if time1 > time2 else 'slower'}")
