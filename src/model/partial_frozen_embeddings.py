...
class PartiallyFrozenEmbedding(nn.Module):
    def __init__(self, frozen_embedding: nn.Embedding, 
                 new_tokens: int, 
                 initializers: list[list[int]],
                 do_init: bool = True):
        super().__init__()
        self.frozen_embedding = frozen_embedding
        self.trainable_embedding = nn.Embedding(new_tokens, frozen_embedding.embedding_dim)
    ...
    def state_dict(self, destination=None, prefix='', keep_vars=False):
        return self.trainable_embedding.state_dict(destination, prefix, keep_vars)
    ...
    def _init_trainable_embedding(self, initializers: list[list[int]]):
        assert len(initializers) == self.num_trainable, "Initializers should be provided for all new tokens"
        old_embeddings_weight = self.frozen_embedding.weight.data.to(torch.float32)

        new_embeddings_weight = _init_new_embeddings(old_embeddings_weight, initializers,
                                                        add_1std_noise_for_empty_init=True)
...
class PartiallyFrozenLMHead(nn.Module):
    def __init__(self, frozen_lm_head: nn.Linear, 
                 new_tokens: int, 
                 initializers: list[list[int]],
                 do_init: bool = True):
        super().__init__()
        ...
        self.trainable_lm_head = nn.Linear(frozen_lm_head.in_features, new_tokens, bias=frozen_lm_head.bias is not None)
    ...
    def state_dict(self, destination=None, prefix='', keep_vars=False):
        return self.trainable_lm_head.state_dict(destination, prefix, keep_vars)
    ...
    def _init_trainable_lm_head(self, initializers: list[list[int]]):
        ...
        new_lm_head_weight = _init_new_embeddings(old_lm_head_weight, initializers,
                                                    add_1std_noise_for_empty_init=True)
        ...
        if self.frozen_lm_head.bias is not None:
            ...
            new_lm_head_bias = _init_new_embeddings(old_lm_head_bias, initializers,
                                                    add_1std_noise_for_empty_init=True)
...
def _init_new_embeddings(old_embeddings, initializers: list[list[int]], add_1std_noise_for_empty_init: bool = True):
...
if __name__ == "__main__":
    ...
    partial_frozen_embedding = PartiallyFrozenEmbedding(frozen_embedding, new_tokens, initializers, do_init)
    ...
    partial_frozen_lm_head = PartiallyFrozenLMHead(frozen_lm_head, new_tokens, initializers, do_init)
